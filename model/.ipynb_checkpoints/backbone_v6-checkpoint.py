#v4+重建
#MRFE+Bi-FPN+DNANet+SLSIoULoss
#新思路先从node4_1开始作为传输，然后到最后一层的mask来再分别与这个传入gated resnet
...
class MultiNet_v6(nn.Module):
    def __init__(self,in_channels,block=ResNetBlock):
        super(MultiNet_v6,self).__init__()
        ...
        self.gatedRes_3=GatedResNet(param_channels[4],param_channels[3])
        self.gatedRes_4=GatedResNet(param_channels[3],param_channels[2])
        self.gatedRes_5=GatedResNet(param_channels[2],param_channels[1])
    ...
    def forward(self,x,tag):
        ...
        edge_out=self.gatedRes_3(self.up_8(out3_3),edge_out)
        edge_out=self.gatedRes_4(self.up_4(out2_3),edge_out)
        edge_out=self.gatedRes_5(self.up_2(out1_3),edge_out)
