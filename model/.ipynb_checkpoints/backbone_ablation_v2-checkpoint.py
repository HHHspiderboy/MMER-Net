# #打破Bi-FPN,使用Dense-nested network的前向传播
...
# class MultiNet_abalation_v2(nn.Module):
#     def __init__(self,in_channels,block=ResNetBlock):
#         super(MultiNet_abalation_v2,self).__init__()
...
#         self.node1_1=self.pileBlock(param_channels[0]+param_channels[1]+param_channels[2],param_channels[1],block,2)
#         self.node2_1=self.pileBlock(param_channels[1]+param_channels[2]+param_channels[3],param_channels[2],block,2)
#         self.node3_1=self.pileBlock(param_channels[2]+param_channels[3]+param_channels[4],param_channels[3],block,2)
...
#         self.node1_3=self.pileBlock(param_channels[0]+param_channels[1]*3+param_channels[2],param_channels[1],block,2)
...
#     def forward(self,x,tag,edge_tag):
#         #添加heatmap+x
...
#         out1_1=self.node1_1(torch.cat([self.down(out0_1),out1_0,self.up_2(out2_0)],dim=1))
#         out2_1=self.node2_1(torch.cat([self.down(out1_1),out2_0,self.up_2(out3_0)],dim=1))
#         out3_1=self.node3_1(torch.cat([self.down(out2_1),out3_0,self.up_2(out4_0)],dim=1))
...
#         out1_2=self.node1_2(torch.cat([self.down(out0_2),out1_0,out1_1,self.up_2(out2_1)],dim=1))
#         out2_2=self.node2_2(torch.cat([self.down(out1_2),out2_0,out2_1,self.up_2(out3_1)],dim=1))
...
#         out1_3=self.node1_3(torch.cat([self.down(out0_3),out1_0,out1_1,out1_2,self.up_2(out2_2)],dim=1))
...
#         if tag:
#             mask4=self.output_4(out4_0)
#             mask3=self.output_3(out3_1)
#             mask2=self.output_2(out2_2)
#             mask1=self.output_1(out1_3)
#             mask0=self.output_0(out0_4)
...
#                 edge_out=self.gatedRes_1(self.up_16(out4_0),edge_out)
#                 # edge_out=self.gatedRes_2(self.up_16(out4_2),edge_out)
#                 edge_out=self.gatedRes_2(self.up_8(out3_1),edge_out)
#                 edge_out=self.gatedRes_3(self.up_4(out2_2),edge_out)
#                 edge_out=self.gatedRes_4(self.up_2(out1_3),edge_out)
#                 edge_out=self.head(edge_out)

#                 return [mask0,mask1,mask2,mask3,mask4],result,edge_out
#             else:
...
class MultiNet_abalation_v2(nn.Module):
    def __init__(self,in_channels,block=ResNetBlock):
        ...
        self.node1_2=self.pileBlock(param_channels[1]*2+param_channels[2],param_channels[1],block,2)
        self.node2_2=self.pileBlock(param_channels[2]*2+param_channels[3],param_channels[2],block,2)
        self.node3_2=self.pileBlock(param_channels[3]*2+param_channels[4],param_channels[3],block,2)
        
        self.node0_2=self.pileBlock(param_channels[0]*2+param_channels[1]*2,param_channels[0],block)
        self.node1_3=self.pileBlock(param_channels[1]*3+param_channels[2],param_channels[1],block,2)
        self.node2_3=self.pileBlock(param_channels[2]*3+param_channels[3],param_channels[2],block,2)
        self.node3_3=self.pileBlock(param_channels[3]*3+param_channels[4],param_channels[3],block,2)
        self.node4_2=self.pileBlock(param_channels[4]*2,param_channels[4],block,2)
    ...
    def forward(self,x,tag,edge_tag):
        ...
        out3_2=self.node3_2(torch.cat([out3_0,out3_1,self.up_2(out4_1)],dim=1))
        out2_2=self.node2_2(torch.cat([out2_0,out2_1,self.up_2(out3_2)],dim=1))
        out1_2=self.node1_2(torch.cat([out1_0,out1_1,self.up_2(out2_2)],dim=1))
        


        out4_2=self.node4_2(torch.cat([out4_0,out4_1],dim=1))
        out3_3=self.node3_3(torch.cat([out3_0,out3_1,out3_2,self.up_2(out4_2)],dim=1))
        out2_3=self.node2_3(torch.cat([out2_0,out2_1,out2_2,self.up_2(out3_3)],dim=1))
        out1_3=self.node1_3(torch.cat([out1_0,out1_1,out1_2,self.up_2(out2_3)],dim=1))
        out0_2=self.node0_2(torch.cat([out0_0,out0_1,self.up_2(out1_2),self.up_2(out1_3)],dim=1))
